...
var DataController 	= {
	...
	index  : function(req, res){
		...
		fs.readFile("public/files/livescores.json", 'utf8', function (err, liveData){
				...
				request(url, function(error, response, html){
					...
					if ($(".cb-schdl").length > 0) {
						if ($(".cb-schdl").find('.cb-lv-main').length > 0) {
							...
							fs.writeFile("public/files/livescores.json", storeData, function(err){
								if (err) {
									console.log(err);
								}
							});
					...
					//res.render("index",{data: data.matches});
				});
			} else {
				res.send({data: data.matches});
				//res.render("index",{data: liveData.matches});
			}
		});
	...
	renderSeries : function(req, res) {
		...
		request(url, function(error, response, html){
			...
			res.send({data:result, moment : moment});
	...
	renderSerieDetails : function(req, res) {
		...
		request(url, function(error, response, html){
			...
			//res.render('series-details',{ result: result });
				...
				if (diff.asMinutes() > 30) {
					request(url, function(error, response, news){
						...
						fs.writeFile(file, storeData, function(err){
							storeData = JSON.parse(storeData);
							res.send({data: storeData.data});
							//res.render('news',{data:storeData.data});
						});
						
					});
				} else {
					res.send({data: storeData.data});
					//res.render('news',{data:newsData.data});
				}
			})
		}
	}
